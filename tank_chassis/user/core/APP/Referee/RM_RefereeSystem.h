...
struct __attribute__((packed)) ext_game_robot_status_t
{
	...
	float bullet_speed_limit;                // 机器人射击初速度上限 (单位: m/s)
...
struct __attribute__((packed)) ext_power_heat_data_t
{
	...
	uint16_t shooter_id1_17mm_cooling_heat;  // 17mm发射机构当前热量
	uint16_t shooter_id1_42mm_cooling_heat;  // 42mm发射机构当前热量
};
...
// 42mm热量诊断：原始字节、memcpy后的字段
extern volatile uint8_t referee_0202_raw_heat[2];
extern volatile uint16_t referee_0202_heat_after_memcpy;
extern volatile uint16_t referee_0202_data_length;
//0x0301 机器人间交互数据
