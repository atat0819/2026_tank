...
{
    // RX is started explicitly by receive()/receive_dma()/receive_dma_idle().
    // Enabling RXNE here conflicts with ReceiveToIdle DMA and can keep the
    // CPU trapped in HAL_UART_IRQHandler before tasks get scheduled normally.
}
